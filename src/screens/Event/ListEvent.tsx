...
import { default_image, MapIconGrey, CalenderGrey } from '../../../const/Png';
...
export default function ListEvent(props) {
	...
	const [GetListEvent, { data, loading, error }] = useLazyQuery(ListEventGQL, {
		...
		variables: {
			...
			type: search.tag,
			cities: props.navigation.getParam('idcity')
				? [props.navigation.getParam('idcity')]
				: null,
			countries: props.navigation.getParam('idcountries')
				? [props.navigation.getParam('idcountries')]
				: null,
			price_start: null,
			price_end: null,
			date_from: null,
...
ListEvent.navigationOptions = ({ navigation }) => ({
	headerTitle: (
