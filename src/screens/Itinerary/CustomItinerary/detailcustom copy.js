...
export default function detailCustomItinerary(props) {
  ...
  const FunAttachment = async (data) => {
      ...
      new Promise((resolve, reject) => {
        RNFetchBlob.config({
          ...
          .then((res) => {
            ...
            setAttachment((attachment) => [...attachment, res.path()]);
            resolve(true);
          })
          .catch((err) => {
            console.log(err);
  ...
  // useEffect(() => {
  //   attachment.length === dataParent.attachment.length ? ()
  // }),
  //   [];
      ...
      : (setprogressAttach(false),
        setTimeout(() => {
          FileViewer.open(downloadFile, {
            ...
            onDismiss: () => RNFetchBlob.fs.unlink(attachment[index]),
          });
        }, 350));
  };
  ...
  useEffect(() => {
    attachment.length === dataParent?.attachment?.length
      ? (setprogressAttach(false), handleAttachment(indexClicked))
      : null;
  }, [attachment]);
                      ...
                      ? item.attachment.map((data, indah) => {
                          return (
                            ...
                            >
                              ...
                              <TouchableOpacity
                                onPress={() => handleAttachment(index, indah)}
                              >
