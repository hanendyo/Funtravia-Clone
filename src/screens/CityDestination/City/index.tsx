...
import { default_image, search_button } from '../../../const/Png';
...
export default function CityDetail(props) {
	...
	const [getPackageDetail, { loading, data, error }] = useLazyQuery(
		CitiesInformation,
		{
			...
			variables: {
				id: props.navigation.getParam('data').city_id,
			},
			context: {
				headers: {
	...
	const _fetchItem = async () => {
		var lat = props.navigation.getParam('data').latitude;
		var long = props.navigation.getParam('data').longitude;
		var kotanya = props.navigation.getParam('data').city_name;
	...
	return (
		<View style={styles.main}>
			...
			<ScrollView
				...
				<RenderUtama
					aktif={actives}
					render={data && data.CitiesInformation ? data.CitiesInformation : {}}
				/>
			</ScrollView>
			<Sidebar
				props={props}
