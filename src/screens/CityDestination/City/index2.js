...
export default function CityDetail(props) {
  ...
  let [tombol, setTombol] = useState(0);
  ...
  const renderScene = ({ route }) => {
    let render =
      dataCity && dataCity.CitiesInformation ? dataCity.CitiesInformation : {};

    let renderjournal =
      dataJournal && dataJournal.journal_by_city
        ? dataJournal.journal_by_city
        : [];
    ...
    if (route.key == "general") {
        ...
        >
          ...
          >
            ...
            >
              {list_journal ? (
                <ImageSlider
                  images={list_journal ? spreadData(list_journal) : []}
                  style={{
      ...
      return (
        <View style={{ padding: 20 }}>
          {render.article_header[index - 1] &&
          render.article_header[index - 1].content.length
            ? render.article_header[index - 1].content.map((i, index) => {
                console.log("item", i);
                if (!i) {
  ...
  const RenderUtama = () => {
    ...
    // console.log("Renderjournal", renderjournal);
    return (
      <ScrollView>
        {/* General information */}
        {render && render.description ? (
