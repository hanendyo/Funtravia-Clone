...
export default function SendCity({ navigation, route }) {
  ...
  const _sendMessage = async (id, index) => {
    ...
    try {
      ...
      if (responseJson) {
        ...
        let dataCity = route.params.city;
        let constain = {
          id: dataCity?.id,
          cover: dataCity?.cover,
          name: dataCity?.name,
        ...
        await fetch(`${CHATSERVER}/api/personal/send`, {
          ...
          body: `room=${
            responseJson.id
          }&type=tag_city&chat=personal&text=${JSON.stringify(
            constain
          )}&user_id=${user.id}`,
        });
