...
export default function Comments(props) {
  ...
  // const [like, setLike] = useState(dataPost.current.liked);
  const [counts, setcounts] = useState(dataPost.current.response_count);
  console.log("counts", counts);
  // console.log("counts", counts);

  const [buttonProses, setButtonProses] = useState(false);
  const [like, setLike] = useState(false);

  const _liked = async (id) => {
    setButtonProses(true);
    if (token) {
      try {
        dataPost.current.liked = true;
        // setcounts(dataPost.current.response_count + 1);
        setcounts(dataPost.current.response_count + 1);
        console.log("console count", dataPost.current.response_count + 1);
        // dataPost.current.response_count = dataPost.current.response_count + 1;
        setButtonProses(false);
        ...
        if (response.data) {
          if (
            ...
            //   dataPost.current.response_count + 1;
            setButtonProses(false);
          } else {
  ...
  console.log("dataPost", dataPost.current);
  ...
  const _unliked = async (id) => {
    ...
    if (token || token !== "") {
      try {
        dataPost.current.liked = false;
        // dataPost.current.response_count = dataPost.current.response_count - 1;
        console.log("count unlike", counts);
        console.log(
          "dataPost.current.response_count",
          dataPost.current.response_count
        );
        setcounts(counts - 1);
        console.log("after count unlike", counts);
            ...
            >
              ...
              >
                {like === false ? (
                  <Button
                    onPress={() => {
                      // _unliked(dataPost.current.id, dataPost.current.user.id)
                      setLike(true), setcounts(counts - 1);
                    }}
                    type="icon"
                    // variant="transparent"
                    position="left"
                  ...
                  <Button
                    onPress={() => {
                      setLike(false),
                        // _liked(dataPost.current.id, dataPost.current.user.id)
                        setcounts(counts + 1);
                    }}
                    type="icon"
                    position="left"
