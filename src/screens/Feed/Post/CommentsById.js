        ...
        try {
          ...
          if (response.data) {
            if (
              ...
              tmpDataPost.comment_count = tmpDataPost.comment_count - 1;
  ...
  const [GetCommentList, { data, loading, error, refetch }] = useLazyQuery(
    CommentList,
    {
      ...
      onCompleted: async (data) => {
        ...
        let temps = { ...data };
        let index = temps.comment.findIndex((k) => k["id"] === comments);
        setIndeks(index);
        comentIndex.current.scrollToIndex({ index: index, animated: true });
      },
    }
  );
