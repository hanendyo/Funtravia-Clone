...
export default function SendJournal({ navigation, route }) {
  ...
  const _sendMessage = async (id, index) => {
    ...
    try {
      ...
      if (responseJson) {
        ...
        let dataJournal = route.params.dataJournal;
        let constain = {
          id: dataJournal?.id,
          cover: dataJournal?.firstimg,
          name: dataJournal?.title,
          description: dataJournal?.firsttxt,
        };
